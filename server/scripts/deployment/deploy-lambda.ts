...
async function getOrCreateRole() {
    try {
        ...
        if (error.name === "NoSuchEntity" || error.name === "NoSuchEntityException") {
            ...
            const createResult = await iamClient.send(new CreateRoleCommand({ RoleName: roleName, AssumeRolePolicyDocument: assumeRolePolicy }));
            const roleArn = createResult.Role!.Arn!;
            await iamClient.send(new AttachRolePolicyCommand({ RoleName: roleName, PolicyArn: "arn:aws:iam::aws:policy/AmazonS3FullAccess" }));
...
async function ensureFunctionUrl() {
    ...
    try {
        ...
        if (response.AuthType !== "NONE") {
            ...
            const updateResponse = await lambdaClient.send(new UpdateFunctionUrlConfigCommand({
        ...
        const createResponse = await lambdaClient.send(new CreateFunctionUrlConfigCommand({
            FunctionName: functionName,
            AuthType: "NONE",
        }));
