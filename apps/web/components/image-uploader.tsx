...
export function ImageUploader({
  ...
  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      ...
      try {
        ...
        const {
          data: { uploadUrl, key },
        } = await axios.post(`${API_URL}/images/presigned-url`, {
          fileName: file.name,
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        {...getRootProps()}
        className={`group cursor-pointer rounded-[1rem] border-2 border-dashed p-12 text-center transition-all ${isDragActive ? "border-primary bg-primary/5" : "border-muted-foreground/10 hover:border-muted-foreground/30"} ${isUploading ? "pointer-events-none opacity-60" : ""} `}
      >
        <input {...getInputProps()} />
